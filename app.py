...
def contar_residuos():
    ...
    st.write(
        "A continuación se mostrará el gráfico de pie con los datos procesados de dicho departamentos "
    )
...
def residuos_ruralyurbano():
    ...
    fig_comparativa_apilada.for_each_trace(lambda trace: trace.update(name=trace.name.replace('POB_RURAL', 'Rural').replace('POB_URBANA', 'Urbano')))
...
# Actualizar la leyenda y los títulos
...
# Actualizar el nombre de las leyendas
    fig_comparativa_grupo.for_each_trace(lambda trace: trace.update(name=trace.name.replace('QRESIDUOS_DOM', 'Domiciliarios').replace('QRESIDUOS_NO_DOM', 'No Domiciliarios')))
